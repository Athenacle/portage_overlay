...
inherit eutils flag-o-matic multilib prefix git-r3 autotools

EGIT_REPO_URI="git@192.168.1.2:/home/git/sandisk/gits/zsh.git"
EGIT_BRANCH="master"
...
DESCRIPTION="UNIX Shell similar to the Korn shell. WangXiao Modified Version."
...
src_unpack(){
	git-r3_fetch ${EGIT_REPO_URI} zsh-${PV}
	git-r3_checkout
}
...
src_prepare() {
	...
	eapply_user
	sed -i "/^VERSION=/s/=.*/=${PV}-WMV./" Config/version.mk || die
	eautoreconf
}
...
src_compile() {
	default
	use doc && emake -C Doc everything
}
